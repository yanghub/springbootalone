...
//		var _showRowCheck = paramObj.showRowCheck != false ? true : false;//是否显示每行的复选框
		...
		var _multiLineShow = paramObj.multiLineShow || false; //自动换行
		var _mergeCell = paramObj.mergeCell || [];//合并单元格
...
//		var _rowSelect = paramObj.rowSelect || null;//行选择事件
    	...
    	var _setBodyColumns = function(){
    		...
    		if(_rows.length > 0){
    			...
    			if (_mergeCell.length > 0) {
    				for (var i = 0; i < _mergeCell.length; i++) {
    					_onMergeCell(_mergeCell[i]);
    				}
    			}
    	...
    	var _onMergeCell = function(columns){
    		var num = [];
    		var arr = $('td[field="'+columns+'"]');
    		if (arr.length > 0) {
    			for (var i = 0; i < arr.length; i++) {
        			var obj = arr[i];
        			var con = obj.innerText;
        			var n = [];
        			n.push(i);
        			for (var j = i+1; j < arr.length; j++) {
        				var dom = arr[j];
        				if (dom.innerText == con) {
        					n.push(j);
        				} else{
        					break;
        				}
        		...
        		for (var i = 0; i < num.length; i++) {
        			var td = num[i];
        			if (td.length > 1) {
        				$('td[field="'+columns+'"]:eq('+td[0]+')').attr('rowspan',td.length);
        				for (var j = 1; j < td.length; j++) {
        					$('td[field="'+columns+'"]:eq('+td[j]+')').hide();
        				}
        			}
        		}
			}
    	...
    	var _addColumn = function(row){
			...
			for(var j=0;j<_columns.length;j++){
				...
				if(_multiLineShow == false){
					...
					var $tempDiv = $('<div style="width:'+(column.width-30)+'px;word-break:break-all; word-wrap:break-word;white-space:pre-wrap;display: inline;margin: 0 auto;">'+value+'</div>');
				}
